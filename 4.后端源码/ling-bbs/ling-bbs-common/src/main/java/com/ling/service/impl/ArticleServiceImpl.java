...
import com.ling.service.tx.ArticleTxService;
...
//    }
    ...
    @Transactional(rollbackFor = Exception.class)
    public void auditArticle(List<String> ids) {
        List<Article> simpleAs = articleMapper.simpleSelectByIds(ids);

        Set<String> toAuditIds = new HashSet<>();   // 记录待审核文章id
        Set<String> toAuditAuthors = new HashSet<>(); // 记录待审核文章作者id
        List<UserMessage> toAuditMessages = new ArrayList<>();  // 记录审核消息

        simpleAs.forEach(sa -> {
            Integer status = sa.getStatus();
            String articleId = sa.getArticleId();

            // 删除文章不能审核，抛异常
            if (status.equals(TargetStatusEnum.DELETED.getStatus())) {
                String errMsg = String.format(CommonMsg.UNABLE_AUDIT_DELETE_ARTICLE, sa.getTitle(), articleId);
                throw new BusinessException(errMsg);
            }

            if (status.equals(TargetStatusEnum.AUDITED.getStatus())) return;    // 跳过已审核

            toAuditIds.add(articleId);
            String author = sa.getUserId();
            toAuditAuthors.add(author);
            toAuditMessages.add(UserMessage.forArticleAudit(author, sa.getArticleId(), sa.getTitle()));
        });

        if (CollectionUtils.isEmpty(toAuditIds)) return;

        // 批量审核
        articleMapper.batchUpdateToAudit(toAuditIds);

        // 更新积分
        Integer points = SysCacheUtil.getSysSettingManager().getSysSetting4Post().getPostPoints();
        toAuditAuthors.forEach(id -> {
            if (points > 0) {
                userPointsRecordService.processUserPoints(id, OperationTypeEnum.POST_ARTICLE.getType(), points);
            }
        });

        // 记录消息
        userMessageService.batchAdd(toAuditMessages);

        // 发送通知(建议异步)
        batchPush(toAuditAuthors);
    }
...
//    }
    ...
    @Transactional(rollbackFor = Exception.class)
    public void delete(List<String> list) {
        List<Article> simpleAs = articleMapper.simpleSelectByIds(list);

        Set<String> toDelIds = new HashSet<>(); // 收集待删除文章id
        Set<String> toAuditAuthors = new HashSet<>(); // 记录待审核文章作者id
        List<UserMessage> toDelMessages = new ArrayList<>();  // 记录删除文章消息

        simpleAs.forEach(sa -> {
            if (sa.getStatus().equals(TargetStatusEnum.DELETED.getStatus())) return;

            toDelIds.add(sa.getArticleId());
            String author = sa.getUserId();
            toDelMessages.add(UserMessage.forArticleDel(author, sa.getArticleId(), sa.getTitle()));
            toAuditAuthors.add(author);
        });

        if (CollectionUtils.isEmpty(toDelIds)) return;

        articleMapper.batchUpdateToDelete(toDelIds);    // 批量删除

        // 记录消息
        userMessageService.batchAdd(toDelMessages);
     ...
     * @param receivers
     */
    private void batchPush(Set<String> receivers) {
        receivers.forEach(id -> {
            long timestamp = System.currentTimeMillis();
            String sign = StrUtil.generateHmacSha256Hex(adminConfig.getSecretKey(), id + timestamp);
            String newUrl = String.format(adminConfig.getInnerUrl() + "/send-msg?userId=%s&timestamp=%d&sign=%s",
                    id, timestamp, sign);
            String res = OkHttpUtil.getRequest(newUrl);
            log.info(res);
        });
    }
}
